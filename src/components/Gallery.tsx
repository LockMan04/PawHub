  ...
  ({ items, onLoadMore, isFetchingMore = false, config }) => {
    ...
    return (
      <main
        id="main-content"
        tabIndex={-1}
        className="relative w-full min-h-screen py-8 sm:py-12 focus:outline-none"
      >
                ...
                {isHeroEnd && onLoadMore && (
                  <div className="w-full flex flex-col items-center justify-center py-16 px-4 select-none">
                    <button
                      ...
                      aria-label={config.labels.summonButton(20)}
                    ...
                    >
                      ...
                      <span className="font-mono text-xs sm:text-sm font-bold tracking-wider uppercase text-neutral-900 dark:text-neutral-100">
                        {isFetchingMore
                          ? config.labels.summoningButton
                          : config.labels.summonButton(20)}
                      </span>
                    </button>
                    <p className="font-mono text-[11px] text-neutral-400 mt-3 tracking-widest uppercase">
                      {config.labels.expandStream}
                    </p>
                  </div>
                )}
